...
public class AsociadoMainControlador {
  ...
  public String buscarAsociado(@RequestParam(required = false) String nCuil, ModelMap modelo) {
    ...
    Long cuilAsoc = Long.parseLong(nCuil.trim());
    ...
    if (asociado == null) {
      ...
      // no se renderiza, son datos ocultos
      String id = asociado.getId();
      Double salMin = asociado.getSalarioMinimo();
      ...
      String cbu = asociado.getCbu();
      Double cuotaSocial = asociado.getCuotaSocial();
      ...
      modelo.put("salMin", salMin);

      modelo.put("nombre", nombre);
      modelo.put("apellido", apellido);
      modelo.put("edad", edad);
      modelo.put("cuotaSocial", cuotaSocial);
