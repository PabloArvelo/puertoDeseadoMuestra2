...
const btnBajaAsoc = document.getElementById("btnBajaAsoc");
...
let bajaAsociadoModal = document.getElementById("bajaAsocDialog");
...
const btnBajaAsociadoSi = document.getElementById("bajaSiBtn");
...
const btnCerrarModalBaja = document.getElementById("bajaNoBtn");
...
btnCerrarModalDatos.addEventListener("click", () => {
    ...
    window.location.href = `/buscar/unoSolo?nCuil=${cuilFuente}`
})
...
// --------------- Abrir y cerrar modal baja Asociado ------------


btnBajaAsoc.addEventListener("click", () => {
    bajaAsociadoModal.showModal();
    overlay.style.display = 'block';

})

btnCerrarModalBaja.addEventListener("click", () => {
    bajaAsociadoModal.close();
    overlay.style.display = "none";
})

btnBajaAsociadoSi.addEventListener("click", () => {
    bajaAsociado(idAsocFuente);

})


function bajaAsociado(idAsoc) {
    fetch('/asociado/baja', {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded' // Enviar datos como form-urlencoded
        },
        body: new URLSearchParams({ idAsoc: idAsoc }) // Parámetro enviado en el body
    })
        .then(response => response.text()) // Como el controlador devuelve un String
        .then(data => {
            bajaAsociadoModal.close();
            overlay.style.display = "none";

            // doy un pequeño tiempo para que cierre el modal previo a lo que sigue
            setTimeout(() => {
                alert(data);                 
        ...
        .catch(error => alert('Error en la baja: ' + error));
}
...
function capturaDatosParaModal() {
    ...
    let salMinFuente = document.getElementById("salMinFuente").value;

    let salMinActualizar = document.getElementById("salMinActualizar");
    salMinActualizar.value = salMinFuente;

    let salMinFotoHidden = document.getElementById("salMinFotoHidden");
    salMinFotoHidden.value = salMinFuente;

    let salMinHabilHidden = document.getElementById("salMinHabilHidden");
    salMinHabilHidden.value = salMinFuente;
    ...
    let cbuFuente = document.getElementById("cbuFuente").innerText;
    console.log("cbu " + cbuFuente)

    let cbuActualizar = document.getElementById("cbuActualizar");
    cbuActualizar.value = cbuFuente;

    let cbuFotoHidden = document.getElementById("cbuFotoHidden");
    cbuFotoHidden.value = cbuFuente;

    let cbuHabilHidden = document.getElementById("cbuHabilHidden");
    cbuHabilHidden.value = cbuFuente;
